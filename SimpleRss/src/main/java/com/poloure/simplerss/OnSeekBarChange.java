...
class OnSeekBarChange implements SeekBar.OnSeekBarChangeListener
{
   ...
   void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser)
   {
      ...
      if(m_settingsFileName.contains("Read Item Opacity"))
      {
         ...
         LayoutFeedItem.s_cardOpacity = opacity;

         LayoutFeedItem.s_titleRead = Color.argb(Math.round(255 * opacity), 0, 0, 0);
         LayoutFeedItem.s_notTitleRead = Color.argb(Math.round(190 * opacity) /* Maybe 66 */, 0, 0,
               0);
      }
   }
