...
public class service_update extends IntentService
{
	...
	protected void onHandleIntent(Intent intent)
	{
		...
		/*final static String UNREAD_ITEM			= getString(R.string.notification_title_singular);
		final static String UNREAD_ITEMS		= getString(R.string.notification_title_plural);
		final static String GROUP_UNREAD		= getString(R.string.all_group);
		final static String GROUP_UNREADS		= getString(R.string.all_group);
		final static String GROUPS_UNREADS			= getString(R.string.all_group);*/
		...
		else if((unread_counts[0] != 0) && intent.getBooleanExtra("NOTIFICATIONS", false))
		{
			...
			String hello;
			if(unread_counts[0] == 1 && group_items == 1)
				hello = String.format("%d group has an unread item", 1);
			else if(unread_counts[0] > 1 && group_items == 1)
				hello = String.format("%d group has unread items", 1);
			else if(group_items > 1)
				hello = String.format("%d groups have unread items", group_items);

			NotificationCompat.Builder not_builder = new NotificationCompat.Builder(this)
					.setSmallIcon(R.drawable.rss_icon)
					.setContentTitle(Integer.toString(unread_counts[0]) + ((unread_counts[0] == 1) ? " Unread Item" : " Unread Items"))
					.setContentText(
					Integer.toString(group_items - 1) +
					((group_items - 1 == 1) ? ((unread_counts[0] == 1) ? " group has an unread item" : " group has unread items.") : " groups have unread items"))
					.setAutoCancel(true);
